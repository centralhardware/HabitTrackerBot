-- Fix an ambiguous column reference in store_param_value (introduced in V44).
--
-- The function declares an OUT parameter named `value`, while the queries in its body compare
-- against the `value` column of `param_values` / `checkin_values`. A bare `value = p_value` is
-- therefore ambiguous between the OUT variable and the column, and PL/pgSQL (default
-- variable_conflict = error) aborts at runtime with:
--   ERROR: column reference "value" is ambiguous
-- the first time a text param value is written.
--
-- The signature is unchanged, so CREATE OR REPLACE is enough (no DROP). The only change from V44 is
-- table-qualifying the three column references so each resolves to the column, not the OUT variable.
