...
object CalendarFeedRepository {
     ...
     * non-deleted tracks, dated on or after [since]. The service groups these by checkin id.
     */
    fun checkinsSince(userId: Long, since: LocalDate): List<CalendarCheckin> =
        sessionOf(DatabaseService.dataSource).use { session ->
            session.run(
                queryOf(
                    """
                    SELECT e.id AS checkin_id, h.name, h.track_type, e.check_date, v.status, read_param_value(v.value, v.value_id, v.value_num) AS value,
